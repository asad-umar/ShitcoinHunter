...
export function resolveModes(): ResolvedModes {
  // Scanner mode
  ...
  const scanner = scannerRaw as ScannerMode;
  ...
  const execution = execRaw as ExecutionMode;

  const resolved: ResolvedModes = {
    scanner,
    execution,
  ...
  logger.info(`[Mode] Scanner: ${scanner.toUpperCase()} | Execution: ${execution.toUpperCase()}`);
  if (resolved.isPaper) {
    logger.info('[Mode] PAPER mode — no real trades will be executed');
  } else {
    logger.warn('[Mode] REAL mode — live trades WILL be executed with real SOL');
  }
